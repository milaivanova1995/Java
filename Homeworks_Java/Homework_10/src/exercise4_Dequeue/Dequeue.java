...
public class Dequeue {
	...
	public static void main(String[] args) {
		...
		for (int i = 0; i < count / 2; i++) {
			...
			dequeue.addFirst(names);
		}
	
		for (int i = count / 2; i < count; i++) {
			System.out.format("Enter the %d name which will be added to the tail: ", i + 1);
			names = input.nextLine();
			dequeue.addLast(names);
		}
		
		System.out.println("The dequeue is: " + dequeue + "\n");

		System.out.format("First - removing elements from the head. One element will left.");
		for (int i = 0; i < (count / 2) - 1; i++) {
			dequeue.removeFirst();
		}
		
		System.out.println("\nSecond - removing elements from the tail. One element will left.");
